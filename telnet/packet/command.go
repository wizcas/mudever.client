...
func NewControlCommandPacket(command protocol.CmdByte) *CommandPacket {
	return &CommandPacket{command, protocol.NoOption}
}

// NewOptionCommandPacket creates a command of a negotiable option
func NewOptionCommandPacket(command protocol.CmdByte, option protocol.OptByte) *CommandPacket {
...
func (p *CommandPacket) Serialize() ([]byte, error) {
	bytes := [3]byte{byte(protocol.IAC), byte(p.Command), byte(p.Option)}
	if p.isControl() {
		return bytes[:2], nil
	}
	return bytes[:], nil
...
func (p *CommandPacket) isControl() bool {
	return p.Option == protocol.NoOption
}
